...
class MetroWindow(QMainWindow):
    ...
    def _init_window(self):
        ...
        self.setWindowTitle(f"Metro RL Environment - Instance {self.env_index}")
        self.setGeometry(100 + self.env_index * 50, 100 + self.env_index * 30, 1500, 800)
    ...
    def refresh_trains(self):
        ...
        for train in self.env.get_all_trains():
            # Access train data for the specific environment instance
            current_node_id = train.state.position[self.env_index].item()
            
            # Convert tensor value to int if needed for display
            self.trainList.addItem(f"Train {train.name}: Node {current_node_id}")
            
            # Update the node color to green to indicate the train is there
            for node_id_key, circle in self.node_items.items():
                if str(node_id_key) == str(current_node_id):
                    circle.setBrush(QBrush(QColor('green')))
                else:
                    circle.setBrush(QBrush(QColor('lightblue')))
        
        # Now for each train, add a point on the timeline
        for train in self.env.get_all_trains():
            # Calculate x position based on current time in this env instance
            x = self.timeline_margin + self.env.steps[self.env_index].item() * self.timeline_tickwidth
            
            # Calculate y position based on the train's position in this env instance
            current_node_id = train.state.position[self.env_index].item()
            
            # Only proceed if the node exists in the mapping
            if current_node_id in self.env.node2segments:
                segment = self.env.node2segments[current_node_id]
                startplat = segment.start_platform
                endplat = segment.end_platform
                
                # Calculate y position on timeline
                if startplat in self.platform_horiz_pos and endplat in self.platform_horiz_pos:
                    y = self.platform_horiz_pos[startplat] - (self.platform_horiz_pos[startplat] - self.platform_horiz_pos[endplat]) * node_in_segment_percentage(
                        self.env.nodes[current_node_id], segment, self.env.segment2nodes
                    )
                    
                    self._add_timeline_point(x, y)
                
                # Update train position in topology scene
                ...
                node_position = self._get_node_positon_in_topology(self.env.nodes[current_node_id])
                trainitem = HoverableGraphicsEllipseItem(
                    self.env.nodes[current_node_id],
                    node_position.x() - 5,
                    node_position.y() - 5,
                    10,
                    10
